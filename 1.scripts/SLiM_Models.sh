#!/bin/bash
#SBATCH --job-name=SLiMRunSAMP_NUM
#SBATCH --partition=popgenom,kamiak,cas,cahnrs
#SBATCH --output=/data/cornejo/projects/e.jimenezschwarzkop/SLiMRecombinationModel/1.scripts/1.log/SLiMRunSAMP_NUM.out
#SBATCH --error=/data/cornejo/projects/e.jimenezschwarzkop/SLiMRecombinationModel/1.scripts/1.log/SLiMRunSAMP_NUM.err
...
#for i in `seq 1 900`; do sed 's/SAMP_NUM/'$i'/g' SLiM_Models.sh | sed 's/PARAMETER_FILENAME/SLiM_Parameters.txt/g' > SLiM_Model_Scripts/SLiM_Model_case$i.sh; done

#!/bin/bash
#SBATCH --job-name=SLiMRun162
#SBATCH --partition=popgenom,kamiak,cas,cahnrs
#SBATCH --output=/data/cornejo/projects/e.jimenezschwarzkop/SLiMRecombinationModel/1.scripts/1.log/SLiMRun162.out
#SBATCH --error=/data/cornejo/projects/e.jimenezschwarzkop/SLiMRecombinationModel/1.scripts/1.log/SLiMRun162.err
...
#for i in `seq 1 900`; do sed 's/162/'$i'/g' SLiM_Models.sh | sed 's/SLiM_Parameters.txt/SLiM_Parameters.txt/g' > SLiM_Model_Scripts/SLiM_Model_case$i.sh; done
...
/data/cornejo/projects/SLiM/slim -s $seed -d N=$N -d POS_1=$POS_1 -d POS_2=$POS_2 -d HI_1=$HI_1 -d HI_2=$HI_2 -d ORD_FREQUENCY_1=$ORD_FREQUENCY_1 -d ORD_FREQUENCY_2=$ORD_FREQUENCY_2 -d PHASE_1=$PHASE_1 -d PHASE_2=$PHASE_2 -d STARTFREQ_1=$N -d STARTFREQ_2=$N -d R=$R 1.scripts/FluctuatingSelection_TwoLocus_SineWave.slim  > 2.model_output/Test_Model_case162_${n}.vcf

directory=$(echo $(lsworkspace -v | tail -n 4 | awk 'NR==1 {print}')'/2.model_output')

mv 2.model_output/Test_Model_case162_${n}.vcf "${directory}/case_162/"
